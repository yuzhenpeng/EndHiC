...
Using default parameters:  "--binsize 100000  --minbinnum 5 --maxbinnum 25 --binnumstep 5",  represents contig end sizes 0.5, 1, 1.5, 2, 2.5 Mb
...
print ENDHIC "mkdir 01.contig_end_contact_results   02.GFA_contig_graph_results  03.cluster_order_orient_results  04.summary_and_merging_results\n";
             `mkdir 01.contig_end_contact_results   02.GFA_contig_graph_results  03.cluster_order_orient_results  04.summary_and_merging_results `;

print ENDHIC "mv z.EndHiC.$ClusterMark.*  $hicpro_raw_matrix_file.*.summary* $hicpro_iced_matrix_file.*.summary*   04.summary_and_merging_results\n";
             `mv z.EndHiC.$ClusterMark.*  $hicpro_raw_matrix_file.*.summary* $hicpro_iced_matrix_file.*.summary*   04.summary_and_merging_results`;

print ENDHIC "mv *.CtgContact *.adjustTransform  *.turningPoint  01.contig_end_contact_results/\n";
             `mv *.CtgContact *.adjustTransform  *.turningPoint  01.contig_end_contact_results/`;

print ENDHIC "mv *.gfa  02.GFA_contig_graph_results/\n";
             `mv *.gfa  02.GFA_contig_graph_results/`;

print ENDHIC "mv *.gfa.cluster  *.gfa.topology  03.cluster_order_orient_results\n";
             `mv *.gfa.cluster  *.gfa.topology  03.cluster_order_orient_results`;
